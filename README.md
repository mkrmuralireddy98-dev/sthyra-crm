# Sthyra CRM — Visual Intelligence for the Built World
...
> `/Users/muralikrishnamuthokuru/.hermes/plans/2026-08-08_090307-sthyra-crm-visual-intelligence-platform.md`
...
├── services/
│   ├── org-service/    # @sthyra-crm/org-service — tenant-scoped orgs (Postgres + in-memory)
│   ├── project-service/# @sthyra-crm/project-service — projects belong to orgs, archive FSM
│   └── user-service/   # @sthyra-crm/user-service — identity, RBAC, opaque tokens
├── apps/
│   └── dashboard/      # @sthyra-crm/dashboard — Next.js 14 App Router (Phase 0 shell)
...
- **Request-ID propagation** via `@sthyra-crm/observability`. Every log line carries `request_id`.
...
- Never `console.log("some debug thing")`. Use `emit('info' | 'warn' | 'error', msg, fields)` from `@sthyra-crm/observability`.
...
Proprietary. © 2026 Sthyra CRM. All rights reserved.
